...
export default async function DashboardPage({ searchParams }: DashboardPageProps) {
  const [data, dbWatchlist] = await Promise.all([
    getHomeData(),
  ...
  const watchedTickers = dbWatchlist
    .filter((item) => item.entityType === "TICKER")
    .map((item) => ({
      title: item.label,
      subtitle: item.subtitle ?? item.symbol ?? "Ticker",
      value: item.notes ?? "Watching",
      tone: ((item.notes ?? "").toLowerCase().includes("buy")
        ? "buy"
        : (item.notes ?? "").toLowerCase().includes("sell")
        ? "sell"
        : "neutral") as "neutral" | "buy" | "sell",
    }));

  const watchedPoliticians = dbWatchlist
    .filter((item) => item.entityType === "POLITICIAN")
    .map((item) => ({
      title: item.label,
      subtitle: item.subtitle ?? "Politician",
      value: item.notes ?? "Watching",
      tone: ((item.notes ?? "").toLowerCase().includes("hot") ||
      (item.notes ?? "").toLowerCase().includes("trend")
        ? "buy"
        ...
        <aside className="hidden border-r border-white/10 bg-black/20 lg:flex lg:flex-col">
          ...
          <div className="flex flex-1 flex-col gap-8 px-4 py-6">
            ...
            <div className="mt-auto rounded-3xl border border-emerald-400/15 bg-emerald-400/10 p-4">
              <p className="text-sm font-medium text-emerald-300">Database-backed watchlist</p>
              <p className="mt-2 text-sm leading-6 text-white/65">
                Watchlist is now loaded from the new Watchlist table while existing core tables remain untouched.
              </p>
            </div>
          </div>
        </aside>
            ...
            <div className="flex flex-col gap-4 px-5 py-4 md:px-8 lg:flex-row lg:items-center lg:justify-between">
              <div>
                ...
                <p className="mt-2 text-sm text-white/50">
                  Querying your existing core tables plus the new Watchlist table.
                </p>
              </div>

              <div className="flex flex-col gap-3 sm:flex-row">
                <div className="rounded-2xl border border-white/10 bg-white/[0.03] px-4 py-3 text-sm text-white/60">
                  Mode: <span className="font-medium text-white">DB-backed watchlist</span>
                </div>
                <Link
                  href="/"
                ...
                <div className="rounded-[28px] border border-white/10 bg-white/[0.03] p-5">
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="mt-1 text-sm text-white/45">Loaded from the Watchlist table.</p>
                    </div>
